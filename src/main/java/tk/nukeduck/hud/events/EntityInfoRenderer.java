	...
	public Entity getMouseOver(Minecraft mc, float partialTick, double length) {
		...
		if(entity != null) {
			if(mc.theWorld != null) {
				...
				//Vec3d vec33 = null;
				float f1 = 1.0F;
				List<Entity> list = mc.theWorld.getEntitiesWithinAABBExcludingEntity(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * distance, vec31.yCoord * distance, vec31.zCoord * distance).expand((double) f1, (double) f1, (double) f1));
				double d2 = blockDist;
