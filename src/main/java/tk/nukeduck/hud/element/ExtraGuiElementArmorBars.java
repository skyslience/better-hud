...
public class ExtraGuiElementArmorBars extends ExtraGuiElement {
	...
	public String generateText(ItemStack item) {
		...
		if(this.showDurability.value) {
			if(durabilityMode.getValue().equals("percent")) {
				parts.add(I18n.format("betterHud.strings.percent", ONE_PLACE.format(value * 100.0)));
			} else {
	...
	public void render(Minecraft mc, ScaledResolution resolution, StringManager stringManager, LayoutManager layoutManager) {
		...
		// TODO why do this?????
		ItemStack[] armor = mc.thePlayer.inventory.armorInventory.toArray(new ItemStack[mc.thePlayer.inventory.armorInventory.size()]);
		...
		for(int i = 0; i < 4; i++) {
			if(isStackValid(armor[i])) {
				mc.getRenderItem().renderItemAndEffectIntoGUI(armor[i], left ? this.bounds.getX() : this.bounds.getX2() - 16, this.bounds.getY() + i * 18);
			}
		}
			...
			if(name || durability) {
				for(int i = 0; i < 4; i++) {
					if(armor[i] != null) {
						...
						String dur;
						if(durabilityMode.getValue().equals("percent")) {
							dur = I18n.format("betterHud.strings.percent", ONE_PLACE.format(value * 100.0));
						} else {
