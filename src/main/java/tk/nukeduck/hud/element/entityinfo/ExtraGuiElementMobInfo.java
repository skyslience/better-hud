	...
	public void renderInfo(EntityLivingBase entity, Minecraft mc, float partialTicks) {
		if(enabled) {
			...
			GL11.glPushMatrix(); {
				...
				mc.ingameGUI.drawString(mc.fontRendererObj, text, 5, 5, Colors.WHITE);
				
				if(isPlayer) {
					String text2 = "(" + entity.getTotalArmorValue() + "/20)";
					mc.ingameGUI.drawString(mc.fontRendererObj, text2, width - 5 - mc.fontRendererObj.getStringWidth(text2), 5, Colors.WHITE);
				}
				...
				if(isPlayer) {
					...
					if(playerObj.getHeldItemMainhand() != null) {
						mc.ingameGUI.drawString(mc.fontRendererObj, "Holding:", 5, height + 5, Colors.WHITE);
						...
						if(is.getEnchantmentTagList() != null && is.getEnchantmentTagList().tagCount() != 0) {
							for(int b = 0; b < is.getEnchantmentTagList().tagCount(); b++) {
								...
								mc.ingameGUI.drawString(mc.fontRendererObj, display, 5 + (100 * (int)(b / 4)), height + 10 + mc.fontRendererObj.FONT_HEIGHT + ((mc.fontRendererObj.FONT_HEIGHT + 2) * (b % 4)), Colors.WHITE);
							}
						}
						mc.ingameGUI.drawString(mc.fontRendererObj, (is.hasDisplayName() ? ChatFormatting.ITALIC : "") + "" + (is.isItemEnchanted() ? ChatFormatting.AQUA : "") + is.getDisplayName(), 10 + 16 + mc.fontRendererObj.getStringWidth("Holding:"), height + 5, Colors.WHITE);
					}
				}
			}
			GL11.glPopMatrix();
