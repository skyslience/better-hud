...
public class ExtraGuiElementPotionBar extends ExtraGuiElement {
	...
	public void render(Minecraft mc, ScaledResolution resolution, StringManager stringManager, LayoutManager layoutManager) {
		...
		for(Iterator<PotionEffect> i = mc.thePlayer.getActivePotionEffects().iterator(); i.hasNext(); xOffset += 16) {
		...
		xOffset = 4;
		for(Iterator<PotionEffect> i = mc.thePlayer.getActivePotionEffects().iterator(); i.hasNext(); xOffset += 16) {
			mc.ingameGUI.drawString(mc.fontRendererObj, I18n.format("potion.potency." + i.next().getAmplifier()).replace("potion.potency.", ""), x + xOffset, y - mc.fontRendererObj.FONT_HEIGHT - 2, Colors.WHITE);
		}
