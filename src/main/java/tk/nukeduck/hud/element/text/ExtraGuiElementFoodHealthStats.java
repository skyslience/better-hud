...
public class ExtraGuiElementFoodHealthStats extends ExtraGuiElementText {
	...
	public void render(Minecraft mc, ScaledResolution resolution, StringManager stringManager, LayoutManager layoutManager) {
		...
		String health = String.valueOf(((int)mc.thePlayer.getHealth()) / 2.0f);
		String food = String.valueOf(mc.thePlayer.getFoodStats().getFoodLevel() / 2.0F);
		...
		mc.ingameGUI.drawString(mc.fontRendererObj, food, center + 95, textY, Colors.WHITE);
		mc.ingameGUI.drawString(mc.fontRendererObj, health, center - 95 - healthWidth, textY, Colors.WHITE);
	...
	protected String[] getText(Minecraft mc) {
		...
		return new String[] {I18n.format("betterHud.strings.saturation",
			Math.round(mc.thePlayer.getFoodStats().getSaturationLevel() * 10) / 10.0f)};
	}
}
