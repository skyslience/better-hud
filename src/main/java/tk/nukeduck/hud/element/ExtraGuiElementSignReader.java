		...
		if(te != null && te instanceof TileEntitySign) {
			if(type.getValue() == "sign.text") {
				...
				if(posMode.index == 0) {
					...
					RenderUtil.renderStrings(mc.fontRendererObj, items, pos2.x, pos2.y, Colors.WHITE, Position.TOP_LEFT);
				}
			} else {
				if(posMode.index == 1) {
					...
					for(ITextComponent ic : ((TileEntitySign) te).signText) {
						...
						mc.fontRendererObj.drawString(s, pos2.x + 44 - mc.fontRendererObj.getStringWidth(s) / 2, pos2.y + 2 + i * (mc.fontRendererObj.FONT_HEIGHT + 2), Colors.BLACK);
						i++;
					}
					...
					for(ITextComponent ic : ((TileEntitySign) te).signText) {
						if(ic == null) ic = new TextComponentString("");
						String s = ic.getFormattedText();
						mc.fontRendererObj.drawString(s, 49 - (mc.fontRendererObj.getStringWidth(s) / 2), layoutManager.get(Position.TOP_LEFT) + 2 + i * (mc.fontRendererObj.FONT_HEIGHT + 2), Colors.BLACK);
						i++;
					}
					...
					if(te instanceof TileEntitySign) {
						for(ITextComponent ic : ((TileEntitySign) te).signText) {
							if(ic == null) ic = new TextComponentString("");
							String s = ic.getFormattedText();
							mc.fontRendererObj.drawString(s, resolution.getScaledWidth() - 49 - (mc.fontRendererObj.getStringWidth(s) / 2), y, Colors.BLACK);
							//i++;
